...
unsigned int luzInt = 0;
unsigned int soloInt = 0;
unsigned int gasInt = 0;
...
void main()
{
    ...
    //Envia o caracter 'L' para o arduino
    UART1_Write('L');
    if(UART1_Data_Ready()){  // Verifica se um dado foi recebido no buffer
        ucRead = UART1_Read();// Lê o dado recebido do buffer.
        WordToStr(ucRead, tempCharLuz); // Converte o dado recebido para string
        Ltrim(tempCharLuz); // Remove os espaços em branco da string
        luzInt=atoi(tempCharLuz); // Converte a string para inteiro
        Delay_ms(50);
    }
    ...
    if(UART1_Data_Ready()){  // Verifica se um dado foi recebido no buffer
        ucRead = UART1_Read();// Lê o dado recebido do buffer.
        WordToStr(ucRead, tempCharSolo); // Converte o dado recebido para string
        Ltrim(tempCharSolo); // Remove os espaços em branco da string
        soloInt=atoi(tempCharSolo); // Converte a string para inteiro
        Delay_ms(50);
    }

    //Envia o caracter 'G' para o arduino
    UART1_Write('G');
    if(UART1_Data_Ready()){  // Verifica se um dado foi recebido no buffer
        ucRead = UART1_Read();// Lê o dado recebido do buffer.
        WordToStr(ucRead, tempCharGas); // Converte o dado recebido para string
		Ltrim(tempCharGas); // Remove os espaços em branco da string
        gasInt=atoi(tempCharGas); // Converte a string para inteiro
        Delay_ms(50);
    }
	...
	while (1)
	{
		...
		if ((soloInt < 50) && (gasInt > 50) && (tempInt >= 20) && (luzInt < 50))
		{
			PORTC.RC1 = 0;
			if (atencaoFlag)
			{
